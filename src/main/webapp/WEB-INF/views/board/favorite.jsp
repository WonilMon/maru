		...
		<div class="container">
			...
			<table>
				...
				<tbody>
					...
					<c:forEach var="list" items="${favoriteList}">
						<tr>
							<td><span class="label">${list.board_info_name }</span></td>
							<td><a
			...
			<div class="wonil_pagination">
				<c:if test="${pageBean.currentPage > 1}">
					<a
						href="${root}board/favorite?user_idx=${loginUserBean.user_idx }&page=${pageBean.currentPage - 1}">&laquo;</a>
				</c:if>
				<c:forEach var="page" begin="${pageBean.min}" end="${pageBean.max}">
					<c:choose>
						<c:when test="${page == pageBean.currentPage}">
							<a
								href="${root}board/favorite?user_idx=${loginUserBean.user_idx }&page=${page}"
								class="active">${page}</a>
						</c:when>
						<c:otherwise>
							<a
								href="${root}board/favorite?user_idx=${loginUserBean.user_idx }&page=${page}">${page}</a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				...
				<c:if test="${pageBean.currentPage < pageBean.pageCnt}">
					<a
						href="${root}board/favorite?user_idx=${loginUserBean.user_idx }&page=${pageBean.currentPage + 1}">&raquo;</a>
				</c:if>
			</div>
