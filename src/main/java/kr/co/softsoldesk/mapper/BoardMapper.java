...
public interface BoardMapper {
	...
	@Insert("insert into content(content_idx, content_subject, content_text, content_file, content_createdAt, content_isAnonymous, content_hashTag, user_idx, board_info_idx) "
			+ "values(content_seq.nextval, #{content_subject}, #{content_text}, #{content_file, jdbcType=VARCHAR}, sysdate, #{content_isAnonymous}, #{content_hashTag}, #{user_idx}, #{board_info_idx})")
	void addContent(ContentBean writeContentBean);

	// 게시글 idx로 게시글 전체 불러오기 (content의 내용들 + user 정보)
	@Select("select u.user_idx as user_idx ,c.content_subject, content_text, to_char(c.content_date, 'YYYY-MM-DD') as content_createdAt, u.user_name as user_name "
	...
	// 게시글 업데이트 update
	@Update("update content_table set content_subject = #{content_subject}, content_text = #{content_text}, content_file = #{content_file, jdbcType=VARCHAR}, content_hashTag = #{content_hashTag} where content_idx = #{content_idx}")
	...
	@Select("SELECT content_idx FROM (SELECT content_idx FROM content where board_info_idx = #{boardInfoIdx} ORDER BY content_createdAt DESC) WHERE ROWNUM <= 10")
	List<Integer> getLatest10ContentIdx(int boardInfoIdx);
