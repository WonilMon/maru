...
public class CompanyService {
	...
	public List<ContentBean> getNoticeList(int board_info_idx, int page) {
		
		int start = (page - 1) * page_list;

		RowBounds rowBounds = new RowBounds(start, page_list);
		
		return companyDao.getNoticeList(board_info_idx, rowBounds);
	}
	
	// 공지사항 - 페이지네이션
	public PageBean getNoticeCnt(int currentPage) {

		int content_cnt = companyDao.getNoticeCnt(5);
	...
	public void addImgFile(String file_name, int sender_idx, int receiver_idx, int content_idx) {
		companyDao.addImgFile(file_name, sender_idx, receiver_idx, content_idx);
	}

	// 나눔관리 - 리스트
	public List<SharingBean> getShareList(int page) {

		int start = (page - 1) * page_list;

		RowBounds rowBounds = new RowBounds(start, page_list);

		return companyDao.getShareList(rowBounds);
	}

	// 나눔관리 - 페이지네이션
	public PageBean getShareCnt(int currentPage) {

		int content_cnt = companyDao.getShareCnt();
		PageBean pageBean = new PageBean(content_cnt, currentPage, page_list, paginationcnt);
	...
	public void updatePoint(String user_nickname) {
		companyDao.updatePoint(user_nickname);
	}
	
	// 나눔관리 - 완료처리
	public void updateAfter(int share_idx) {
		companyDao.updateAfter(share_idx);
	}
}
